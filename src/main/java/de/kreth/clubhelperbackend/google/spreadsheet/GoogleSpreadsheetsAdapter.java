...
class GoogleSpreadsheetsAdapter extends GoogleBaseAdapter {
	...
	public GoogleSpreadsheetsAdapter()
	...
	protected void checkRefreshToken(String serverName) throws IOException {
		...
		if (service == null) {
			throw new IllegalStateException(
					"Spread Sheet Service not initialized!");
		}
	}

	private BatchUpdateSpreadsheetResponse sendRequest(Request request,
			Boolean includeSpreadsheetInResponse) throws IOException {
		...
		BatchUpdate batch = service.spreadsheets().batchUpdate(SPREADSHEET_ID,
				content);
		BatchUpdateSpreadsheetResponse response = batch.execute();
		return response;
	}

	public List<Sheet> getSheets(String serverName)
			throws IOException, InterruptedException {
		if (log.isTraceEnabled()) {
			log.trace("Loading Sheets");
		}
		Spreadsheet sheet = loadSheet(serverName);
		return sheet.getSheets();
	}

	private Spreadsheet loadSheet(String serverName) throws IOException {
		...
		try {
			...
			sheet = spreadsheets.get(SPREADSHEET_ID).setIncludeGridData(false)
					.execute();

		} catch (IOException e) {
			if (log.isDebugEnabled()) {
				log.debug("Error fetching SpreadSheed, trying token refresh",
						e);
			}
	...
	public Sheet dublicateTo(String serverName, String originalTitle,
	...
	public ValueRange setValue(String sheetTitle, int column, int row,
			ValueRange content) throws IOException {
	...
	public ValueRange setValue(String range, ValueRange content)
			throws IOException {
		if (log.isDebugEnabled()) {
			log.debug("Setting value of " + range + " to " + content);
		}
		Update updateExecutor = service.spreadsheets().values()
				.update(SPREADSHEET_ID, range, content);
		UpdateValuesResponse response = updateExecutor
				.setValueInputOption("RAW").execute();
		return response.getUpdatedData();
	}

	public ValueRange getValues(String sheetTitle, String range)
			throws IOException {
